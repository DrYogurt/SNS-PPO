...
def parse_args():
    ...
    parser.add_argument("--wandb-project-name", type=str, default="ppo-sns-test",
...
class Actor(nn.Module):
    ...
    def reset(self):
        self.hidden = torch.zeros(args.num_envs,self.hid_size)
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor = Actor(np.array(envs.single_observation_space.shape).prod(), args.hidden_size, envs.single_action_space.n)
        
    def get_value(self, x):
        return self.critic(x)

    def get_action_and_value(self, x, action=None, hidden=None):
        logits,hidden = self.actor(x,hidden)
        probs = Categorical(logits=logits)
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action), probs.entropy(), self.critic(x), hidden
...
if __name__ == "__main__":
    ...
    hiddens = torch.zeros((args.num_steps, args.num_envs,agent.actor.hid_size)).to(device) #TODO: 64 is hard coded as the size of the hidden layer - change?
    ...
    for update in range(1, num_updates + 1):
        ...
        b_hiddens = hiddens.reshape((-1,agent.actor.hid_size))
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue, _ = agent.get_action_and_value(b_obs[mb_inds], b_actions.long()[mb_inds],hidden=b_hiddens[mb_inds])
